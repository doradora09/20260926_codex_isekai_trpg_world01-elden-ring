...
function resolve(initial=false){const hash=location.hash;let id=data.firstDayId,notice='';if(/^#scene-1-\d+(?:-[a-z]+)?$/.test(hash))id=hash.slice(7);else if(/^#day-\d+$/.test(hash)){const day=Number(hash.slice(5)),entry=data.byDay[day];if(entry){id=entry.id;const s=allScenes.find(x=>x.id===id);if(!entry.covered)notice='DAY '+day+'の独立した記録はありません。次に記録された話を表示しています。';else if(s.startDay!==s.endDay)notice='DAY '+day+'は、'+s.dayLabel+'を描くこの話に含まれます。'}}let i=visibleIndex(id);if(i>=0&&scenes[i].id!==id)history.replaceState(null,'',href(scenes[i]));if(i<0){i=scenes.findIndex(s=>s.id===data.firstDayId);notice='指定された話が見つからないため、DAY1を表示しています。'}render(i,notice,null,initial)}
