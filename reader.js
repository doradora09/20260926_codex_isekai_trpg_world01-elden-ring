...
function nav(id,s,direction){const a=$(id),toCredits=!s&&direction>0;const label=s?(direction<0?'← 前の話\n':'次の話 →\n')+s.title:toCredits?'エンドロールへ →':'物語のはじまり';a.textContent=label;a.setAttribute('aria-disabled',String(!s&&!toCredits));a.setAttribute('aria-label',label.replace('\n',' '));a.title=label.replace('\n',' ');if(id.endsWith('Top'))a.textContent=direction<0?'‹':'›';if(s)a.href=href(s);else if(toCredits)a.href='credits.html';else a.removeAttribute('href')}
